...
def calculate_velocity(panda, cur_joint, tar_position, tar_orientation):
    ...
    # Remove any random noise added to the velocity 
...
class PandaSim(object):
    ...
    def random_select_object(self,exclude=None):
        objects = list(self.objectUids)
        if exclude is not None and exclude in objects:
            objects.remove(exclude)
        if not objects:
            raise ValueError("No objects left to select.")
...
def calc_velocity(ee_trajectory, time_step):
    # Extract positions from the ee_trajectory
    positions = np.array([entry[:3] for entry in ee_trajectory])

    # Calculate the change in position
    delta_positions = np.diff(positions, axis=0)

    # Calculate velocity using the provided time step
    velocity = delta_positions / time_step

    return velocity
    
def add_velocity_noise(velocity, noise_std):

    # Generate random noise with the same shape as velocity
    noise = np.random.normal(loc=0, scale=noise_std, size=velocity.shape)

    # Add the generated noise to the original velocity
    velocity_with_noise = velocity + noise

    return velocity_with_noise

def non_stationary_noise(step, initial_std=0.1, std_increase=0.01, mean_shift=0.05):
    """
    Generate non-stationary noise where both mean and standard deviation change over time.
    ...
    :param mean_shift: Mean shift per step.
    :return: Noise value for the current step.
    """
    current_std = initial_std + step * std_increase
    current_mean = step * mean_shift
    noise = np.random.normal(loc=current_mean, scale=current_std)
    return noise

def add_non_stationary_noise(velocity, step, initial_std=0.1, std_increase=0.01, mean_shift=0.05):
    noise = np.array([non_stationary_noise(step, initial_std, std_increase, mean_shift) for _ in range(velocity.shape[0])])
    velocity_with_noise = velocity + noise
    return velocity_with_noise
...
if __name__=="__main__":
    ...
    ee_vel_log = []
    ee_goal_log = []
    ee_goal_pos_log = []    # i log the goal pos now !!!
    ee_potential_goal_pos_log = [] #log all the potential goal positions
    joint_pos_log = []
    joint_vel_log = []
    ...
    while(1):
        ...
        i = 0 
        ee_trajectory = []
        ee_trajectory_velocities=[]
        goal_pos_log=[]     # Log the goal pos for each iteration !!!
        potential_goal_pos=[]   # Log all the potential goal positions 
        # Set velocity to 0 for the first iteration
        velocity = np.zeros(3)        #should this be initialized here or inside the loop?
        
        
        noise_std=0.1    #what values should I use?
        joint_pose = []
        joint_vel = []
        
        # Initialize lists to store previous and current positions
        previous_positions = []
        current_positions = []
        ...
        # Record all potential goal positions at the start
        for obj_id in panda.objectUids:
            obj_pos, _ = p.getBasePositionAndOrientation(obj_id)
            potential_goal_pos.append(obj_pos)

        matrix, grasp_orn = panda.random_approaching_orn(tar_id)
        pos,orn = p.getBasePositionAndOrientation(tar_id)
        current_goal_id = pos # Log the current goal pos
        #print("tar_id:",tar_id)
        ...
        while(1):
            ...
            #goal_id_log.append(current_goal_id)  # Log the current goal ID
        ...
        # Add a flag to track whether the robot has already changed its mind
        has_changed_mind = False
        
        previous_goal_position = pos  # Initialize the variable to store the previous goal position
        current_goal_position = pos  # Initialize the variable to store the current goal position
        change_of_intent = np.random.random() < 0.6
        
        min_steps_before_change = 10  # Minimum number of steps before considering changing the goal

            
        while(1):
            
            if not has_changed_mind and change_of_intent and len(ee_trajectory)> min_steps_before_change:  # Only consider changing target if the robot hasn't already changed its mind
                #change_of_intent = np.random.random() < 0.3
            
                if np.random.random() < 0.1:
                    try: # Try to select a new target
                        new_tar_id = panda.random_select_object(exclude=tar_id)
                        tar_id=new_tar_id
                        # Store the previous goal position before updating it
                        previous_goal_position = current_goal_position
                        # Update the current goal position
                        current_goal_position, _ = p.getBasePositionAndOrientation(tar_id)
                        # Update the approach orientation for the new target
                        matrix, grasp_orn = panda.random_approaching_orn(tar_id)
                        print("change of intent")
                        current_goal_id=current_goal_position  # Log the current goal pos!!!!
                        #goal_id_log.append(current_goal_id)  # Log the current goal pos
                        # Print the change in goals
                        #print(f"Changed goal from {previous_goal_position} to {current_goal_position}")
                        has_changed_mind = True  # Set the flag to True after changing the target
                    except ValueError:
                        print("No objects left to select.")
            ...
            goal_pos_log.append(current_goal_id)  # Log the current goal ID
            
            #print("EE_DATA:" ,ee_data)
            
            if not ee_trajectory:  # Check if it's the first iteration
                velocity_with_noise=velocity
            else:
                # Calculate the velocity  
                velocity = calc_velocity([ee_trajectory[-1], ee_data], steps*timeStep)
            
                #add noise
                #velocity_with_noise= add_non_stationary_noise(velocity,len(ee_trajectory) , initial_std=0.1, std_increase=0.01, mean_shift=0.05)
                velocity_with_noise = add_velocity_noise(velocity, noise_std)
          
            # Ensure velocity remains 1D
            velocity_with_noise = np.squeeze(velocity_with_noise)
                
            #print("VELOCITY",velocity_with_noise)
            
           
            ee_trajectory_velocities.append(velocity_with_noise)
            ee_trajectory.append(ee_data)
            #print(ee_trajectory)
            
            # Save previous and current positions once per simulation as a tuple
            goal_positions=[previous_goal_position, current_goal_position]
        ...
        ee_vel_log.append(np.array(ee_trajectory_velocities))
        ee_goal_log.append(np.array(current_positions))
        ee_goal_log.append(goal_positions)
        ee_goal_pos_log.append(np.array(goal_pos_log))
        ee_potential_goal_pos_log.append(np.array(potential_goal_pos))
        #print(ee_trajectory)
        
        # Check if positions and velocities have the same length
        if len(ee_trajectory) != len(ee_trajectory_velocities):
            print("Error: Positions and velocities have different lengths.")
        
        
        index += 1
        #print("previous_goal_position:",previous_goal_position)
        #print("current_goal_position:",current_goal_position)
        #print("goal id log:",goal_id_log)
        #print("potential_goal_pos:",potential_goal_pos)
        ...
        if len(ee_log)>total_data_num:
            ...
            data_dict['ee_vel_log']=ee_vel_log
            data_dict['ee_goal_log']=ee_goal_log
            data_dict['ee_goal_id_log']=ee_goal_pos_log    #this is the one with the goal pos for all time steps
            data_dict['ee_potential_goal_pos_log']=ee_potential_goal_pos_log
            with open('Traj2.5k-dataset-std0.1-changeIntent.json', 'wb') as fp:
