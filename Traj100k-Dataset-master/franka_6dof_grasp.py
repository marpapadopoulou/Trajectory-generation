...
def cosine_noise(step, amplitude=0.1, frequency=0.05):
    """
    Generate cosine noise.
    
    :param step: Current simulation step.
    :param amplitude: Amplitude of the cosine wave.
    :param frequency: Frequency of the cosine wave.
    :return: Noise value for the current step.
    """
    noise = amplitude * np.cos(2 * np.pi * frequency * step)
    return noise

def add_cosine_noise(velocity, step, amplitude=0.1, frequency=0.05):
    noise = np.array([cosine_noise(step, amplitude, frequency) for _ in range(velocity.shape[1])]).T
    velocity_with_noise = velocity + noise
    return velocity_with_noise

def random_walk_noise(steps, noise_std=0.1):
    """
    Generate random walk noise.
    
    :param steps: Number of steps in the random walk.
    :param noise_std: Standard deviation of the noise.
    :return: Array of random walk noise values.
    """
    noise = np.random.normal(0, noise_std, steps)
    random_walk = np.cumsum(noise)
    return random_walk

def add_random_walk_noise(velocity, noise_std=0.1):
    steps = velocity.shape[0]
    noise = random_walk_noise(steps, noise_std)
    noise = np.tile(noise.reshape(-1, 1), (1, velocity.shape[1]))
    velocity_with_noise = velocity + noise
    return velocity_with_noise

def quadratic_noise(step, a=0.01, b=0, c=0):
    """
    Generate quadratic noise.
    ...
    :return: Noise value for the current step.
    """
    noise = a * step**2 + b * step + c
    return noise

def add_quadratic_noise(velocity, step, a=0.01, b=0, c=0):
    noise = np.array([quadratic_noise(step, a, b, c) for _ in range(velocity.shape[1])]).T
    velocity_with_noise = velocity + noise
    return velocity_with_noise

def random_walk_noise(step, initial_value=0, noise_std=0.1):
    """
    Generate random walk noise.
    
    :param step: Current simulation step.
    :param initial_value: Starting value of the random walk.
    :param noise_std: Standard deviation of the noise at each step.
    :return: Noise value for the current step.
    """
    noise = initial_value
    for _ in range(step):
...
def add_random_walk_noise(velocity, initial_value=0, noise_std=0.1):
    steps = velocity.shape[0]
    noise = np.array([random_walk_noise(step, initial_value, noise_std) for step in range(steps)])
    noise = np.tile(noise.reshape(-1, 1), (1, velocity.shape[1]))
    velocity_with_noise = velocity + noise
    return velocity_with_noise
...
#                velocity_with_noise = add_cosine_noise(velocity, len(ee_trajectory), amplitude=0.1, frequency=0.05)
                velocity_with_noise = add_quadratic_noise(velocity, len(ee_trajectory), a=0.01, b=0, c=0)
#                velocity_with_noise = add_random_walk_noise(velocity, initial_value=0, noise_std=0.1)
#                velocity_with_noise = add_random_walk_noise(velocity, noise_std=0.1)
        ...
        # check alignemnt of the positions and the velocities
        # for i in range(min(5, len(ee_trajectory_velocities))):  # Check the first 5 data points or all available if less than 5
        #    position = np.array(ee_trajectory[i][:3])
        #    velocity = np.array(ee_trajectory_velocities[i])
        ...
        if len(ee_log)>total_data_num:
            ...
            with open('Traj20k-dataset-non-stationary-quad.json', 'wb') as fp:
