...
if __name__=='__main__':
    ...
    print("Press 'H' for homing, 'A' to approach, 'D' to move down, 'G' to grasp, 'R' to return to ready pose, 'Q' to quit.")

    while True:
        keys = p.getKeyboardEvents()
        print(keys)  # Debugging: print detected keys

        if ord('H') in keys and keys[ord('H')] & p.KEY_WAS_TRIGGERED:
           
            print("Homing gripper...")  # Debugging
            while not panda.gripper_homing():
                setCameraOnRobotWrist(p, panda.panda, 11)
                p.stepSimulation()

        if ord('A') in keys and keys[ord('A')] & p.KEY_WAS_TRIGGERED:
            print("Approaching...")  # Debugging
        ...
        if ord('G') in keys and keys[ord('G')] & p.KEY_WAS_TRIGGERED:
            print("Grasping...")  # Debugging
            while not panda.grasp():
                setCameraOnRobotWrist(p, panda.panda, 11)
                p.stepSimulation()

        if ord('R') in keys and keys[ord('R')] & p.KEY_WAS_TRIGGERED:
            print("Returning to ready pose...")  # Debugging
            while not panda.ready_pose():
                setCameraOnRobotWrist(p, panda.panda, 11)
                p.stepSimulation()

        if ord('Q') in keys and keys[ord('Q')] & p.KEY_WAS_TRIGGERED:
            print("Quitting...")  # Debugging
            break
