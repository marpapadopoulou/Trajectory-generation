...
def add_ar1_noise(velocity, prev_noisy_velocity, alpha, noise_std):
    # Generate Gaussian noise
    noise = np.random.normal(loc=0, scale=noise_std, size=velocity.shape)
    
    # Apply the AR(1) process
    noisy_velocity = alpha * prev_noisy_velocity + noise
...
if __name__=="__main__":
    ...
    alpha = 0.9  # Autoregressive coefficient
    noise_std = 0.1  # Standard deviation of the Gaussian noise
    prev_noisy_velocity = np.zeros(3)  # Initialize previous noisy velocity
    ...
    while(1):
        ...
        while(1):
            ...
            if not ee_trajectory:  # Check if it's the first iteration
                velocity_with_noise = prev_noisy_velocity
            else:
                # Calculate the velocity  
                velocity = calc_velocity([ee_trajectory[-1], ee_data], steps*timeStep)
            
                #add noise
                velocity_with_noise=add_ar1_noise(velocity, prev_noisy_velocity, alpha, noise_std)
                prev_noisy_velocity = velocity_with_noise
        ...
        if len(ee_log)>total_data_num:
            ...
            with open('test_ARmodel.json', 'wb') as fp:
